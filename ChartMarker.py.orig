...
if not support.fetch("wx","Error: wxPython is required for this tool!"):
  exit(1)
support.fetch("numpy","Warning: NumPy is required for drawing target - function disabled")
...
parser.add_option("-B", "--border", type="float", dest="border_size",
                  help="Set size of border that activates scrolling during dragging",
                  metavar="PIX")
parser.add_option("-F", "--format", dest="fmt",
                  help="The output format of the chart polygon. Default is x1,y1,x2,y2,x3,y3,x4,y4",
                  metavar="STRING")
...
class ChartMarker(wx.Frame):
  def __init__( self ):
    ...
    if math.fabs(1-opts.scale) > 0.001:
      self.image = self.image.Scale(int(self.image.GetWidth()*opts.scale),
      ...
      with open(opts.target,"r") as tin:
        ...
        if found_boxes:
          ...
          while len(line) > 0:
            T = line.split()
            if T[0] == "F":
              self.fiducials = [wx.RealPoint(float(T[3]),float(T[4])),
              ...
              mark.origin = wx.RealPoint(float(T[7]),float(T[8]))
              mark.offset = wx.RealPoint(float(T[9]),float(T[10]))
          ...
          for mark in self.marks:
            ...
            maxX = max(maxX,mark.origin.x+mark.Nx*mark.offset.x)
            maxY = max(maxY,mark.origin.y+mark.Ny*mark.offset.y)
    ...
    if self.mouseState_drag:
      if self.selected_handle is None:
        if self.button_status == 0:
          self.image_position = self.image_position +mousepos -self.mouseState_zpos
      else:
        ...
        if mousepos.x < opts.border_size:
          self.image_position = self.image_position +wx.Point(opts.border_size-mousepos.x,0)
          mousepos = mousepos +wx.Point(opts.border_size-mousepos.x,0)
        if mousepos.y < opts.border_size:
          self.image_position = self.image_position +wx.Point(0,opts.border_size-mousepos.y)
          mousepos = mousepos +wx.Point(0,opts.border_size-mousepos.y)
        if mousepos.x > size.GetWidth() -opts.border_size:
          self.image_position = self.image_position +wx.Point(size.GetWidth() -opts.border_size -mousepos.x,0)
          mousepos = mousepos +wx.Point(size.GetWidth() -opts.border_size -mousepos.x,0)
        if mousepos.y > size.GetHeight() -opts.border_size:
          self.image_position = self.image_position +wx.Point(0,size.GetHeight() -opts.border_size -mousepos.y)
          mousepos = mousepos +wx.Point(0,size.GetHeight() -opts.border_size -mousepos.y)
      self.mouseState_zpos = mousepos
      needsRefresh = True
      
    else:
      
      if opts.handle_size < mousepos.x and mousepos.x < 2*opts.handle_size and\
            opts.handle_size < mousepos.y and mousepos.y < 2*opts.handle_size:
      ...
      for idx in xrange(len(self.target_area)):
        ...
        d = mousepos -wx.Point( (corner.x+corner2.x)/2,
  ...
  def OnPaint(self,event):
    ...
    g.DrawBitmap( self.bitmap, self.image_position.x, self.image_position.y, False )
    ...
    g.SetPen(wx.RED_PEN)
    g.DrawPolygon( self.target_area, self.image_position.x, self.image_position.y )
    ...
    for idx in xrange(len(self.target_area)):
      ...
      corner2 = self.target_area[(idx+1)%4]
      g.DrawRectangle( (corner.x+corner2.x)/2 +self.image_position.x -opts.handle_size/2,
                       (corner.y+corner2.y)/2 +self.image_position.y -opts.handle_size/2,
                       opts.handle_size, opts.handle_size )
    ...
    center.x /= 4
    center.y /= 4
    g.DrawRectangle( center.x +self.image_position.x -opts.handle_size/2,
                     center.y +self.image_position.y -opts.handle_size/2,
                     opts.handle_size, opts.handle_size )
    
    center2 = center + center + self.target_area[0] + self.target_area[1]
    center2.x /= 4
    center2.y /= 4
    g.DrawRectangle( center2.x +self.image_position.x -opts.handle_size/2,
                     center2.y +self.image_position.y -opts.handle_size/2,
                     opts.handle_size, opts.handle_size )
    ...
    if self.selected_handle is not None:
      ...
      if self.selected_handle < 4:
        ...
        g.DrawRectangle( corner.x/2 +self.image_position.x -opts.handle_size/2,
                         corner.y/2 +self.image_position.y -opts.handle_size/2,
                         opts.handle_size, opts.handle_size )
